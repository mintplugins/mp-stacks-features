...
if( !defined( 'MP_STACKS_FEATURES_VERSION' ) )
	define( 'MP_STACKS_FEATURES_VERSION', '1.0.0.7' );
...
function mp_stacks_features_include_files(){
	 ...
	 */
		 ...
		 * enqueue scripts
		 */
		require( MP_STACKS_FEATURES_PLUGIN_DIR . 'includes/misc-functions/admin-enqueue-scripts.php' );
		...
		if ( function_exists('mp_stacks_developer_textdomain') ){
			function mp_stacks_featured_add_active( $required_add_ons ){
				$required_add_ons['mp_stacks_features'] = 'MP Stacks + Features';
				return $required_add_ons;
			}
			add_filter( 'mp_stacks_active_add_ons', 'mp_stacks_featured_add_active' );
		}
		 ...
		 * Misc Functions
		 */
		require( MP_STACKS_FEATURES_PLUGIN_DIR . 'includes/misc-functions/misc-functions.php' );
